...
skip = 3
L_min, L_max = 1e-20, 1e-1
L_vals = np.logspace(-20, -1, 100)
# ----------------------------------------------------
# ODE system
# ----------------------------------------------------
def f(val, r, n, Gamma, K, Q, L):
    m, omega = val
    ft = omega
...
def update(frame):
    ...
    for n in n_vals[::skip]:
        for Gamma in Gamma_values[::skip]:
            try:
                ...
                sol = odeint(f, yv0, rp, args=(n, Gamma, K, Q, L))
    ...
    ax.set_title(
        rf"$\Lambda = {float(digit):.2f} \times 10^{{{int(exponent)}}}$", y = 1.04
    )
...
# Create animation
# ----------------------------------------------------
ani = FuncAnimation(fig, update, frames=len(L_vals), interval=1000, blit=False)

writer = FFMpegWriter(fps=10, metadata=dict(artist='Me'), bitrate=1800)
ani.save("L_animation_CM.mp4", writer=writer)